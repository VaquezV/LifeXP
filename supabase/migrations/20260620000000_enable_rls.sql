...
drop policy if exists domain_scores_insert on public.domain_scores;
...
drop policy if exists domain_scores_update on public.domain_scores;
create policy domain_scores_update on public.domain_scores for update to authenticated
  using (exists (select 1 from public.checkins c where c.id = domain_scores.checkin_id and c.user_id = auth.uid()))
  with check (exists (select 1 from public.checkins c where c.id = domain_scores.checkin_id and c.user_id = auth.uid()));
drop policy if exists domain_scores_delete on public.domain_scores;
...
drop policy if exists preset_habits_select on public.preset_habits;
...
drop policy if exists preset_badges_select on public.preset_badges;
